...
public:
  ...
  /**
   * Constructor
   * @param[in] coeffs  coefficients of polynomial y=p(x), along which the state (x, y) is supposed to evolve
   */
  ...
  /**
   * Evaluation operator
   * @param[out] fg     cost and contraints to be used for non-linear optimization
   * @param[in]  vars   state and control values at times t, ..., t+N-1
   */
  ...
  )
{
  ...
  // number of independent variables (N timesteps using N - 1 actuations)
